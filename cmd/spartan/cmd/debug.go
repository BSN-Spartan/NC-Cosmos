...
$ %s debug pubkey '{"@type":"/cosmos.crypto.secp256k1.PubKey","key":"AurroA7jvfPd1AadmmOvWM2rJSwipXfRf8yD6pLbA2DJ"}'
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cmd.Println("Hex Consensus Address:", pk.Address())
			cmd.Println("Bech32 Consensus Address:", sdk.ConsAddress(pk.Address()).String())
			cmd.Println("Hex Consensus PubKey:", hex.EncodeToString(pk.Bytes()))

			pkStr, err := bech32.ConvertAndEncode(sdk.GetConfig().GetBech32ConsensusPubPrefix(), legacy.Cdc.MustMarshal(pk))
			if err != nil {
				return err
			}
			cmd.Println("Bech32 Consensus PubKey:", pkStr)
			return nil
		},
	}
}
