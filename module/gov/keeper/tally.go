...
func (keeper Keeper) Tally(ctx sdk.Context, proposal types.Proposal) (passes bool, burnDeposits bool, tallyResults types.TallyResult) {
	...
	keeper.sk.IterateBondedValidatorsByPower(ctx, func(index int64, validator stakingtypes.ValidatorI) (stop bool) {
		val, ok := validator.(nodetypes.Validator)
		if !ok {
			return true
		}

		operator, err := sdk.AccAddressFromBech32(val.Operator)
		if err != nil {
			return true
		}

		operatorValAddr := sdk.ValAddress(operator.Bytes())
		currValidators[operatorValAddr.String()] = types.NewValidatorGovInfo(
			operatorValAddr,
	...
	for _, val := range currValidators {
		...
		votingPower := sdk.NewDecFromInt(val.BondedTokens)
	...
	percentVoting := totalVotingPower.Quo(totalBondedTokens.ToDec().Abs())
